...
Module G250_Strategy_X_Chain
  ...
  Public Sub Strategy_GCx(U_temp(,) As String)
    ...
    For Cdd As Integer = 1 To 9   ' Les candidats sont traités au fur et à mesure de 1 à 9
      ...
      ' A ce niveau j'ai tous les chemins possibles composés du lien initial et éventueelement inversé
      ' il manque le lien W valide
      For Each road As List(Of GLink_Cls) In GAllRoads
        Jrn_Add_Orange("=== Nouvelle Road ===")
        For Each link As GLink_Cls In road
          Jrn_Add_Orange(
            $"Cel = ({U_Coord(link.Cel(0))}, {U_Coord(link.Cel(1))}),  " &
            $"Type = {link.Type}, " &
            $"Unité = {link.Unité}, " &
            $"Cdd = {String.Join(",", link.Cdd)}, " &
  ...
  Public Sub Strategy_GCx_2(U_temp(,) As String)
    If Xap Then Jrn_Add(, {Proc_Name_Get()})
